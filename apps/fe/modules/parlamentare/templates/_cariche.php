...
<?php foreach ($cariche as $id => $carica): ?>
  <?php $presenze = $carica['Astenuto'] + $carica['Contrario'] + $carica['Favorevole'] + $carica['Partecipante votazione non valida'] + 
	                    $carica['Presidente di turno'] + $carica['Richiedente la votazione e non votante'] + $carica['Voto segreto'] ?>
  <?php $numero_votazioni = $carica['Astenuto'] + $carica['Contrario'] + $carica['Favorevole'] + $carica['Partecipante votazione non valida'] + 
	                        $carica['Presidente di turno'] + $carica['Richiedente la votazione e non votante'] + $carica['Voto segreto'] +
  ...
  <tr>
    ...
    <td>
      <?php foreach($gruppi as $nome => $gruppo): ?>
        ...
        <?php if($presenze!=0): ?>
		  <?php printf('%d volte su %d voti (%01.2f %%)', $ribelle_count, $presenze, number_format($ribelle_count/$presenze *100,2)) ?><br />
	    <?php else: ?>
		  <?php print('0 volte su 0 voti (0%)') ?><br />
		<?php endif; ?>
	  <?php endforeach; ?>  	
    </td>
	<td><?php printf('%d volte su %d voti (%01.2f %%)', $presenze, $numero_votazioni, number_format($presenze/$numero_votazioni *100,2)) ?></td>
	<td><?php printf('%d volte su %d voti (%01.2f %%)', $carica['Assente'], $numero_votazioni, number_format($carica['Assente']/$numero_votazioni *100,2)) ?></td>
	<td><?php printf('%d volte su %d voti (%01.2f %%)', $carica['In missione'], $numero_votazioni, number_format($carica['In missione']/$numero_votazioni *100,2)) ?></td>
    <td>
