        ...
        <div class="W100_100">
            <div class="W25_100 float-right" style="width:23%">
                ...
                <?php if ($carica) : ?>
                    ...
                    <?php if ($ramo == 'camera') : ?>
                        <?php
                            $url = "http://www.camera.it/leg17/29?tipoAttivita=&tipoVisAtt=&tipoPersona=&shadow_deputato=" .
                                $carica->getParliamentId() . "&idLegislatura=17"
                        ?>
                        <?php echo link_to(
                            'la sua pagina su ' .
                            image_tag(
                    ...
                    <?php elseif ($ramo == 'senato') : ?>
                        ...
                        <?php
                            $url = "http://www.senato.it/loc/link.asp?tipodoc=sattsen&leg=17&id=" .
                ...
                <div class="float-container">
                    ...
                    <div class="politician-more-info">
                        ...
                        <?php if (isset($descrizione_cariche)): ?>
                            ...
                            <?php include_partial(
                                'altreCariche', array('descrizione_cariche' => $descrizione_cariche)
                            ); ?>
        ...
        <div class="sixcol">
            ...
            <?php else: ?>
                ...
                <div class="float-container" style="padding:2px 10px 10px 20px;">
                    ...
                    <?php if ($carica->getDataInizio() > "2013-05-15") : ?>
            ...
            <!-- BOX PRESENZE -->
            <h5 class="subsection-alt" style="margin-top:0px;">
                Presenze in <?php echo $nvotazioni ?> votazioni elettroniche
            </h5>
            ...
            <div style="display: none;" class="help-box float-container">
                <div class="inner float-container">
                    ...
                    <p>I dati sulle presenze si riferiscono alle votazioni elettroniche che si svolgono
                        nell'Assemblea di Camera e Senato dall'inizio della legislatura.
                        Le presenze dunque non si riferiscono a tutte le possibili attivit&agrave; parlamentari
                        (lavori preparatori nelle Commissioni) ma solo al totale delle presenze nelle votazioni
                        elettroniche in Aula.

                        <span style="BACKGROUND-COLOR: yellow">Nota: Con assenza si intendono i casi di non partecipazione al voto:
                                sia quello in cui il parlamentare è fisicamente assente (e non in missione)
                                sia quello in cui è presente ma non vota e non partecipa a determinare
                                il numero legale nella votazione</span>
                    </p>
                </div>
            </div>
            ...
            <div class="meter-bar float-container">
                ...
                <div class="meter-bar-container">
                    <div class="meter-label">
                        <strong class="green"><?php echo number_format($presenze_perc, 2) ?>%</strong>&nbsp;
                        (<?php echo number_format($presenze, 0) ?>)
                    </div>

                    <label>presenze:</label>

                    <div class="green-meter-bar">
                        <div style="left: <?php echo number_format($presenze_media_perc, 2) ?>%;" class="meter-average">
                            <label>valore medio: <?php echo number_format($presenze_media_perc, 2) ?>%</label>&nbsp
                        </div>
                        <div style="width: <?php echo number_format($presenze_perc, 2) ?>%;" class="meter-value">
                            &nbsp;</div>
                    </div>
                </div>
                ...
                <div class="meter-bar-container">
                    <label>assenze*:</label>
                    <div class="meter-label">
                        <strong class="red"><?php echo number_format($assenze_perc, 2) ?>%</strong>&nbsp;
                        (<?php echo number_format($assenze, 0) ?>)
                    </div>
                    <div class="red-meter-bar">
                        <div style="left: <?php echo number_format($assenze_media_perc, 2) ?>%;" class="meter-average">
                            <label>valore medio: <?php echo number_format($assenze_media_perc, 2) ?>%</label>&nbsp;
                        </div>
                ...
                <div class="meter-bar-container">
                    ...
                    <div class="meter-label">
                        <strong class="blue">
                            <?php echo number_format($missioni_perc, 2) ?>%
                        </strong>&nbsp;(<?php echo number_format($missioni, 0) ?>)
                    </div>
                    <div class="blue-meter-bar">
                        <div style="left: <?php echo number_format($missioni_media_perc, 2) ?>%;" class="meter-average">
                            <label>valore medio: <?php echo number_format($missioni_media_perc, 2) ?>%</label>&nbsp;
                        </div>
                ...
                <span style="font-weight:normal; padding-top:5px; float:left; text-align:left;">
                        <?php if ($carica->getPoliticoId() == 687289) : ?>
                            <strong>N.B. L'On. Quintarelli risulta assente per motivi di salute
                                dal 29 aprile 2013.<br/>
                            </strong>
                        <?php elseif ($carica->getPoliticoId() == 687398) : ?>
                            <strong>N.B. L'On. Romano risulta assente per motivi di salute
                                dal 28/05/2013 al 01/06/2013.<br/>
                            </strong>
                        <?php endif; ?>

                    <span style="BACKGROUND-COLOR: yellow">
                            * Con assenza si intendono i casi di non partecipazione al voto: sia quello
                            in cui il parlamentare è fisicamente assente (e non in missione)
                            sia quello in cui è presente ma non vota e non partecipa a determinare
                            il numero legale nella votazione</span>
                        Purtroppo attualmente i sistemi di documentazione dei resoconti di Camera e Senato
                            non consentono di distinguere un caso dall'altro.
                            I regolamenti non prevedono la registrazione del motivo dell'assenza
                            al voto del parlamentare. Non si può distinguere, pertanto,
                            l'assenza ingiustificata da quella, ad esempio, per ragioni di salute.
                        </span>
            </div>
