...
<?php foreach ($pager->getResults() as $votazione): ?>
  <?php $totale_ribelli = $votazione->getRibelliCount() ?>
  <tr>
    ...
    <td class="<?php echo($totale_ribelli>0 ? 'ribelli' : '') ?>"><?php echo $totale_ribelli ?></td>
  </tr>
<?php endforeach; ?>
  <tr>
    <td colspan="10" align="center">
      <?php if ($pager->haveToPaginate()): ?>
        <?php echo link_to('<<', 'default/index?page=1') ?>
        <?php echo link_to('<', 'default/index?page='.$pager->getPreviousPage()) ?>
        <?php foreach ($pager->getLinks() as $page): ?>
          <?php echo link_to_unless($page == $pager->getPage(), $page, 'default/index?page='.$page) ?>
        <?php endforeach; ?>
        <?php echo link_to('>', 'default/index?page='.$pager->getNextPage()) ?>
          <?php echo link_to('>>', 'default/index?page='.$pager->getLastPage()) ?>
        <?php endif; ?>    	
    </td>	
  </tr>
  <tr>
