...
class OppIndiceAttivitaPeer extends OppIndicePeer
{
    ...
    public static function calcola_indice_politico($carica_id, $legislatura, $data = '', $verbose = '', $atti_ids = array(), $emendamenti_ids = array())
    {
        ...
        if ($verbose)
            printf("\ntotale atti presentati: %d - punteggio: %7.2f - punteggio totale: %7.2f\n", $n_atti, $d_punteggio, $punteggio);
        ...
        $punteggio += $d_punteggio;
        if ($verbose)
            printf("totale atti relazionati: %d - punteggio: %7.2f - punteggio totale: %7.2f\n", $n_atti_relazionati, $d_punteggio, $punteggio);
        ...
        if ($verbose)
            printf("totale punteggio per tutti gli emendamenti: %7.2f - punteggio totale: %7.2f\n", $em_punteggio, $punteggio);
        ...
        $d_interv_punteggio = self::calcolaPunteggioInterventi($carica_id, $data, $interventi_node, $verbose);
        ...
        if ($verbose)
            printf("totale punteggio per partecipazione: %7.2f - punteggio totale: %7.2f\n", $d_punteggio, $punteggio);
    ...
    public static function calcolaIndiceAtto($carica_id, $atto_id, $tipo_atto_id, $data, $atto_node, $verbose = false, $mode = 'presentazione')
    {
        ...
        // l'iter è calcolato in modo differente per primi firmatari e relatori:
        // - per i primi firmatari: sempre
        // - per i relatori: solo per atti non assorbiti, mentre l'iter per gli unificati è computato
        //   fino all'unificazione esclusa, e completamente per l'atto unificato principale
        if ($mode == 'presentazione' || $mode == 'relazione') {
            ...
            // il computo del punteggio per l'iter non parte nel caso di atti assorbiti
            if ($mode == 'relazione' and $is_absorbed)
                return $punteggio;
            ...
            if ($mode == 'relazione') {
                ...
                if ($verbose)
                    printf("    ultimo relazionato da me: %s\n", $ultimo_atto_relazionato_in_navetta_da_me ? 'y' : 'n');
                if (!$ultimo_atto_relazionato_in_navetta_da_me) return $punteggio;
            }


            // gli iter sono marcati come assorbiti o unificati, se è il caso
            if ($is_absorbed) {
                if ($verbose)
            ...
            foreach ($itinera_atto as $iter_atto) {
                ...
                if ($mode == 'relazione') {
                    $dd_punteggio = self::getPunteggio($tipo_atto, $passaggio, 1);
                    if ($verbose)
                        printf("    iter %s (%s %s) %7.2f\n",
                            $passaggio, $iter_atto['data'], 'magg (relatore)', $dd_punteggio);
                } else {
                    $dd_punteggio = self::getPunteggio($tipo_atto, $passaggio, $carica_in_maggioranza_al_passaggio);
                    if ($verbose)
                        printf("    iter %s (%s %s) %7.2f\n",
                ...
                // --- bonus maggioranza ---
                if ($mode != 'relazione' && ($passaggio == 'approvato' || $passaggio == 'approvato_camera')) {
            ...
            if ($diventato_legge_in_altri_rami && is_null($is_absorbed) && (is_null($is_unified) || $is_unificato_main)) {
                ...
                if ($mode == 'relazione')
                    $d_punteggio += $dd_punteggio = self::getPunteggio($tipo_atto, 'diventato_legge', 1);
                else
