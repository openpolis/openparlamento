...
class OppAttoHasIterPeer extends BaseOppAttoHasIterPeer
{
  ...
  public function getItineraAttoDataRS($atto_id, $data)
  {
    ...
    $sql = sprintf("select ai.data, i.id as iter_id from opp_iter i, opp_atto_has_iter ai where ai.atto_id=%d and ai.data < '%s' and ai.iter_id=i.id and i.id in (4, 7, 58, 57, 54, 52, 36, 30, 22, 25, 20, 16, 11, 13, 40, 41, 42, 45, 50, 56) group by iter_id order by ai.id;",
                   $atto_id, $data);
