...
$t->ok($first_news->getTipoAtto() == 'D', 'TipoAtto');
...
$t->diag('Create a non-final votation');
...
$n_related_news = count($related_news);
$first_news = $related_news[0]; 
$t->ok($n_related_news == 2, 'Now the news related to the act are two');
...
$t->ok(NewsPeer::hasGroupVotation($data, $ramo) == true, 'Group news of Home rilevance');
$t->ok(NewsPeer::hasGroupVotation($data, $ramo, $obj->getOppTipoAtto()->getTipo()) == true, 'Group news of List relevance');
$t->ok(NewsPeer::hasGroupVotation($data, $ramo, $obj->getOppTipoAtto()->getTipo(), $obj->getId()) == true, 'Group news, of Single relevance, related to the act');
dumpNews($t, "Group news generated by the votation", NewsPeer::getAllGroupVotations());
...
$related_news = getRelatedNews($obj);
$n_related_news = count($related_news);
$t->ok($n_related_news == 2, 'The related news are still two');

$t->diag('Create a final votation, the same day');
$vot = OppVotazionePeer::retrieveByPK(9898);
$vot_att = new OppVotazioneHasAtto();
$vot_att->setOppVotazione($vot);
$vot_att->setOppAtto($obj);
$vot_att->save();
$related_news = getRelatedNews($obj);
$n_related_news = count($related_news);
$t->ok($n_related_news == 3, 'There are three related news now');
...
$related_news = getRelatedNews($obj);
$n_related_news = count($related_news);
$t->ok($n_related_news == 7, 'Seven news related to the act');
...
$related_news = getRelatedNews($obj);
$n_related_news = count($related_news);
$t->ok($n_related_news == 8, 'Eight news related to the act');
...
$int->setOppAtto($obj);
$int->save();
dumpNews($t, "Group news generated by the intervention", NewsPeer::getAllGroupInterventions());

$related_news = getRelatedNews($obj);
$n_related_news = count($related_news);
$t->ok($n_related_news == 9, 'Nine news related to the act');
dumpNews($t, "News related to the act", $related_news);

$pol_related_news = getRelatedNews($pol);
$n_pol_related_news = count($pol_related_news);
$t->ok($n_pol_related_news == 6, 'Six news related to the politician');
dumpNews($t, "News related to the politician", $pol_related_news);

dumpNews($t, "All group news (before removal of act)", NewsPeer::getAllGroupNews());
...
$related_news = getRelatedNews($obj);
$t->ok(count($related_news) == 0, 'The news directly related to the act were all removed along with the removal of the act');


dumpNews($t, "All group news (after removal of act)", NewsPeer::getAllGroupNews());
...
function dumpNews($t, $msg, $news_to_dump)
{
  ...
  $t->diag(sprintf("%-6s %-12s %-20s %-54s %-8s %1s %-6s %-10s %-4s %-6s %-16s", 
                   "Id", "RelModel", "GeneratorModel", "GeneratorPKs", "Date", "P", "TAtto", "DataPres", "Ramo", "SedeI", "CreatedAt"));
  foreach ($news_to_dump as $news)
  {
    $t->diag(sprintf("%-6d %-12s %-20s %-54s %-8s %1d %-6s %-10s %-4s %-6d %-16s", 
                     $news->getId(), 
                     ...
                     $news->getTipoAtto(), $news->getDataPresentazioneAtto('Ymd'), 
