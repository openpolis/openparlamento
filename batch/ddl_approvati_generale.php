...
#main
foreach (array(15,16) as $x)
{
	$c = new Criteria();
	$c->add(OppAttoHasIterPeer::ITER_ID, array($x), Criteria::IN);
	#$c->add(OppAttoHasIterPeer::ATTO_ID, 15753);
	$modifiche=OppAttoHasIterPeer::doSelect($c);
	#echo count($modifiche);

	foreach ($modifiche as $modifica)
	{
		if (!in_array($modifica->getOppAtto()->getId(),$arr_leggi))
		{
			$arr_leggi[]=$modifica->getOppAtto()->getId();
			$atto_origine=get_pred($modifica);
			$date_approvazioni=get_approvazioni_pred($modifica);
			if ($atto_origine->getIniziativa()==NULL)
				$iniziativa=2;
			else
				$iniziativa=$atto_origine->getIniziativa();
			echo "http://parlamento17.openpolis.it/singolo_atto/".$modifica->getOppAtto()->getId()."*".$modifica->getOppAtto()->getTitolo()."*".$atto_origine->getOppTipoAtto()->getDenominazione()."*".$iniziativa."*".$atto_origine->getDataPres()."*".$date_approvazioni."*".$modifica->getData()."\n";
		}
	}
}
